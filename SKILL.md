...
xianchou generate video --mode first --prompt "让参考图动起来" --project-id <id> --first-frame-url "url" --poll
...
3. 提交视频生成任务时传入 `--project-id`、动态获取的模型参数、prompt 和所需参考图 URL。
4. 使用 `--poll` 或 `xianchou generate task <task_id> --poll` 轮询任务。
5. 任务成功后 settle，记录或下载返回的视频 URL。

## 重要规则

1. **CLI 只调用 `/api/cli/*`**，不要直接调用 `/run`、`/canvas`、`/toolbar-image`、`/toolbar-video` 等内部接口。
...
4. **视频 URL 参数必须可被献丑服务访问**：本地文件先上传或转换为平台可访问 URL，不要把本地路径当作 URL 传给生成接口。
